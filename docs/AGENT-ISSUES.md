...
> **Epic 状态**：**✅ 已关闭** · 2026-07-19 — 见 [`HANDOFF-M5.md`](./HANDOFF-M5.md) §5  
> **进度核验**：脚本 `m5`/`m4` + 真机定位/天气/健身房；整阶段 M5 工程轨见同文件 §7  
...
摘要：shared Zod（Location、ToolName、SSE tool 事件）、`COACH_AGENT_ENABLED` 环境变量；**不改**运行时行为。
...
| **估时**       | 1.5–2 天                                             |
| **状态**       | **已完成** · 2026-07-19 核验（api test 含 geo mock） |
...
摘要：`UserLocationSnapshot` + `PUT/GET /users/me/location`；可选逆地理；Coach GPS 静默持久化；手测见 Issue §7（`Invoke-RestMethod`）。
...
| **估时**       | 1–2 天                                        |
| **状态**       | **已完成** · 2026-06-16（脚本与文档已交付）   |
| **详细文档**   | [**AGENT-10.md**](./issues/agent/AGENT-10.md) |
| **接手**       | [**HANDOFF-M5.md**](./HANDOFF-M5.md)          |

摘要：`m5-agent-acceptance.ps1`、`HANDOFF-M5.md`、Epic 关闭检查表（**正式勾选仍待本机跑脚本**）。
...
1. **M5 已精简关闭**（Agent Epic + E1）；E2/E3 见 [`HANDOFF-M5.md`](./HANDOFF-M5.md) §7，按需再开
2. **可选**：[`MEAL-QUALITY-01`](./issues/agent/MEAL-QUALITY-01.md) 饮食计划与食物库对齐
3. **M6 / Phase 2**：社区、报告等（Roadmap）

---

_版本：v2.1 · 进度核验同步 · 2026-07-19_
