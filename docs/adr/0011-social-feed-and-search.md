...
- 发帖时校验每个 id：`ownerUserId === 当前用户` && `status === 'READY'` && `mime` 以 `image/` 开头；任一不满足返回 `SOCIAL_MEDIA_INVALID`。
- 读 feed 时把整页所有 `mediaIds` **摊平成一个数组做一次 `findMany`**，禁止逐帖查询（N+1）。
- 展示 URL 沿用现有做法：`storage.presignGet(objectKey, TTL)` 逐个签发（本地 HMAC 计算，无网络往返），与 `users.service.ts` 头像、`reports.service.ts` 页图同构。
- 媒体被置为 `DELETED` 时留下的悬空 id 在渲染层静默过滤，不报错。
- `UploadScope` 新增 `POST_IMAGE`，objectKey 前缀 `post/{userId}/`，仅允许图片 mime。

### 4. 点赞：只做 `LIKE`，幂等 `PUT` / `DELETE`

`ReactionKind` 保留四值，API 层只接受 `LIKE`。不做长按表情选择器、不做按 kind 分组计数。
...
评论列表是 `where postId, deletedAt: null order by createdAt asc` 的一次分页。`parentId` 照常写入，但**只用于渲染「回复 @某某」前缀**，不做树形聚合、不做「查看全部 N 条回复」的二级分页。
...
为对冲「多一个中间件」的代价，后端封装 `SearchProvider` 接口，与 PRD §6 已有的 `StorageProvider` / `LLMProvider` / `OAuthProvider` 同构，作为第四个可移植性接口：
...
两个实现：`MeiliSearchProvider`、`PgSearchProvider`（后者即 `contains` + `mode: 'insensitive'`，十余行）。由 `SEARCH_PROVIDER` 环境变量**显式**选择，并在启动日志打印当前 provider。

**严禁隐式 fallback**：Meili 调用失败不得静默转 `ILIKE`，否则「搜不到」时无法区分是索引未同步还是服务不可用。失败即抛 `SOCIAL_SEARCH_UNAVAILABLE`。

`PgSearchProvider` 的存在同时解决了两件事：CI 不必新增服务；service 单测可注入假 provider。

### 9. 检索只存倒排，展示数据一律回 Postgres

`searchPosts` / `searchUsers` **只返回 id 数组**，服务端再用 `findMany({ where: { id: { in: ids } } })` 回库取权威数据并按 ids 顺序重排。
...
触发重索引的事件因此收敛为三个：帖子创建、帖子删除 / 审核拒绝、用户昵称变更。

**隐私红线**：`User.phone` 不进任何索引、不进任何社交 DTO。搜索不得以任何形式命中手机号。
...
- job payload：`{ op: 'INDEX_POST' | 'DELETE_POST' | 'INDEX_USER', id }`。
- 客户端**不直连** Meilisearch（master key 必然泄露，且绕过后端就无法过滤软删、`PRIVATE` 与 `REJECTED`），搜索统一走 `GET /v1/social/search` 代理。
- 必须配套**全量 reindex 脚本**（`pnpm --filter api reindex:social`）：这是索引漂移与 Meili 数据卷丢失的唯一兜底路径。

### 11. 搜索分页：把 offset 编码进 cursor

Meilisearch 是 offset/limit 分页，与仓库 `{ cursor, limit }` + `nextCursor` 的契约形状冲突。**不为搜索单开一套契约**：cursor 直接编码 offset 字符串，`nextCursor = String(offset + limit)`，返回结果不足 limit 时 `nextCursor = null`。
...
**唯一增强**：`orderBy: [{ createdAt: 'desc' }, { id: 'desc' }]`。Prisma 的 cursor 分页依据游标行的排序字段值定位，`createdAt` 并列时会漏行或重复；feed 是全站聚合，撞并列的概率远高于「用户看自己记录」的既有列表。对外契约一字不变。

### 13. 内容审核：先发后审

落地 ARCHITECTURE §9 承诺的「关键词 + LLM 兜底」。

1. **同步关键词**：发帖 / 发评论时过一遍 `packages/shared/src/constants/social-moderation.ts` 的词表，命中直接 400 `SOCIAL_CONTENT_REJECTED`，不落库。
2. **异步 LLM**：帖子以 `moderation = PENDING` 创建并**立即可见**，同时 enqueue `AiRun(SOCIAL_MODERATE)`；worker 调 DeepSeek 判定后回写 `APPROVED` 或 `REJECTED` + `moderationReason`，`REJECTED` 时 enqueue 索引删除。
3. **可见性判据**：feed / 搜索 / 详情对他人过滤 `moderation !== 'REJECTED'`（即 `PENDING` 与 `APPROVED` 都可见）；作者本人可见全部。

不选先审后发：自己发帖要等数秒才可见，且 worker 一挂所有帖子永久卡在不可见状态。

**开关与配额**：

- `SOCIAL_MODERATION_ENABLED=false` 时发帖直接 `APPROVED` 且不入队（无 LLM 额度时社区仍完全可用）。
- 审核**落 `AiRun`** 保持 PRD §5.3「所有 AI 调用都有成本记录」，但**不调用 `assertDailyLimit`**——它是系统行为，计入 PRD §7 的用户每日配额会导致发几个帖就耗光计划生成额度。
...
新建 `apps/mobile/src/features/social/`，并将错置于 `features/coach/` 的占位屏一并清理。

### 16. 验收数据：seed 造内容 + 脚本验链路

社交是首个**依赖他人数据**才能验收的功能：广场、搜索、点赞他人、评论他人在单账号下全是空列表。两者分工明确：

- **seed**（`packages/db/prisma/seed-social.ts`，独立于产品数据 seed）：3–5 个演示用户 + 数十条帖子 + 若干评论点赞，幂等 upsert，`reset-db` 后一条命令回满。图片无法凭空写入 MinIO，故 seed 帖子以纯文本为主。
- **验收脚本**（`scripts/social-acceptance.ps1`，仿 `m4-acceptance.ps1`）：证明「发帖 → 审核 → 索引 → 搜到 → 点赞 → 计数正确」端到端通。
...
多切片各带一次 migration 会让并行开发时的时间戳顺序打架，而这些列加了不用零代价。

## Consequences

- **正面**：feed / 计数 / 分页全部走既有范式，无新概念；`SearchProvider` 让 Meili 成为可拆卸组件，CI 与单测不依赖容器；索引只存倒排使点赞评论不触发重索引，同步事件收敛为三个；「帖子不可变」消除了审核与索引链路的更新态竞态。
- **负面**：docker-compose 新增第四个服务（Meilisearch），本地资源占用上升；索引与库之间存在秒级一致性窗口，且需要维护 reindex 脚本；`SEARCH_PROVIDER` 造成本地与预期环境的搜索质量差异，排查问题需先确认 provider；审核为每篇帖子引入一次 LLM 调用成本。
- **对既有**：`Post` / `Comment` 加列 + `AiTaskType` 加值需 migration；`UploadScope` 增 `POST_IMAGE` 需同步 `uploads.service` 的 mime 白名单分支；`packages/shared/src/schemas/phase2/social.ts` 由占位 schema 升级为完整请求 / 响应契约；`env.schema.ts` 新增 4 个变量；ARCHITECTURE §3 的 `(social)` 占位落地为实模块。
- **明确不做**：`Follow` 表与关注流、`Notification` 表与站内通知、`PartnerProfile` 伙伴匹配、举报与后台治理、帖子编辑、多表情、结构化打卡附件、话题标签。

## References

- PRD §3.2 P2/P3/P4、§5.4、§6、§7；ARCHITECTURE §3、§4、§8.3、§9
- ADR [0002](./0002-rest-zod-contract.md)（REST + Zod 契约）、[0003](./0003-modular-monolith-with-worker.md)（Worker）、[0004](./0004-presigned-upload.md)（presigned 上传）、[0009](./0009-health-report-analysis.md)（`mediaIds String[]` 松关联先例）
